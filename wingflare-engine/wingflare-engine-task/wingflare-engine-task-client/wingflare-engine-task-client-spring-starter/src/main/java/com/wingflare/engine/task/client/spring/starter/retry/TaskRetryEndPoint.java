package com.wingflare.engine.task.client.spring.starter.retry;
...
import com.wingflare.engine.task.client.retry.cache.FutureCache;
import com.wingflare.engine.task.client.retry.cache.RetryerInfoCache;
import com.wingflare.engine.task.client.retry.callback.future.CallbackTaskExecutorFutureCallback;
import com.wingflare.engine.task.client.retry.callback.future.RetryTaskExecutorFutureCallback;
...
import com.wingflare.engine.task.client.retry.executor.RemoteCallbackExecutor;
import com.wingflare.engine.task.client.retry.executor.RemoteRetryExecutor;
import com.wingflare.engine.task.client.retry.loader.TaskRetrySpiLoader;
...
import com.wingflare.engine.task.client.retry.timer.StopTaskTimerTask;
import com.wingflare.engine.task.client.retry.timer.TimerManager;
import com.wingflare.engine.task.common.core.enums.StatusEnum;
import com.wingflare.api.retry.IdempotentIdContext;
...
@TaskEndPoint
@Validated
public class TaskRetryEndPoint implements Lifecycle {
    ...
    public Result<Boolean> dispatch(@Valid DispatchRetryRequest request) {
        ...
        if (Objects.isNull(retryerInfo)) {
            TaskEngineLog.REMOTE.error("Scene [{}] configuration does not exist, please check if your scene and executor exist", request.getSceneName());
        ...
        RetryArgSerializer retryArgSerializer = TaskRetrySpiLoader.loadRetryArgSerializer(request.getSerializerName());
        ...
        try {
            ...
            TaskEngineLog.REMOTE.error("Parameter parsing exception args:[{}]", request.getArgsStr(), e);
        ...
        TaskEngineLog.REMOTE.info(" Retry task:[{}] scheduled successfully.", request.getRetryTaskId());
    ...
    public Result<Boolean> callback(@Valid RetryCallbackRequest callbackDTO) {
        ...
        try {
            ...
            if (Objects.isNull(retryerInfo)) {
                TaskEngineLog.REMOTE.error("Scene [{}] configuration does not exist, please check if your scene and executor exist", callbackDTO.getSceneName());
                return new Result<>(0, "Callback failed", Boolean.FALSE);
            }

            RetryArgSerializer retryArgSerializer = TaskRetrySpiLoader.loadRetryArgSerializer(callbackDTO.getSerializerName());
            ...
            TaskEngineLog.REMOTE.error("Parameter parsing exception", e);
            return new Result<>(0, "Callback failed", Boolean.FALSE);
        }
        ...
        TaskEngineLog.REMOTE.info(" Callback task:[{}] scheduled successfully.", callbackDTO.getRetryTaskId());
        return new Result<>(Boolean.TRUE);
    }
    ...
    public Result<String> idempotentIdGenerate(@Valid
        ...
        Assert.notNull(retryerInfo,
                () -> new TaskRetryClientException("Retry information does not exist for scene:[{}] executorName:[{}]", scene, executorName));

        Method executorMethod = retryerInfo.getMethod();

        RetryArgSerializer retryArgSerializer = TaskRetrySpiLoader.loadRetryArgSerializer(generateRetryIdempotentIdRequest.getSerializerName());
        ...
        try {
            ...
            throw new TaskRetryClientException("Parameter parsing exception", e);
        }
        ...
        try {
            ...
            TaskEngineLog.LOCAL.error("Idempotent ID generation exception: {}, {}", scene, argsStr, exception);
            throw new TaskRetryClientException("idempotentId generation exception: {}, {}", scene, argsStr);
        }
    ...
    public Result<Object> deserialize(@Valid RetryArgsDeserializeRequest retryDeserializeRequest) {
        ...
        RetryerInfo retryerInfo = RetryerInfoCache.get(scene, executorName);
        RetryArgSerializer retryArgSerializer = TaskRetrySpiLoader.loadRetryArgSerializer(retryDeserializeRequest.getSerializerName());
    ...
    public void start() {
        ...
        TaskProperties.ThreadPoolConfig threadPoolConfig = taskProperties.getRetry().getDispatcherThreadPool();
