package com.wingflare.engine.task.server.common.strategy;
...
public class WaitStrategies {
    ...
    public enum WaitStrategyEnum {
        ...
        WaitStrategyEnum(Integer type, WaitStrategy waitStrategy) {
            this.type = type;
        ...
        public static WaitStrategyEnum getWaitStrategyEnum(int type) {
            ...
            // 兜底为默认等级策略
            throw new TaskCommonException("Wait strategy type does not exist. [{}]", type);
        }
    ...
    private static final class CronWaitStrategy implements WaitStrategy {
        ...
        public Long computeTriggerTime(WaitStrategyContext context) {
            ...
            try {
                ...
                Assert.notNull(nextValidTime, () -> new TaskServerException("Expression error: {}", context.getTriggerInterval()));
                return DateUtils.toEpochMilli(nextValidTime);
            } catch (ParseException e) {
                throw new TaskServerException("Parsing CRON expression exception [{}]", context.getTriggerInterval(), e);
            }
