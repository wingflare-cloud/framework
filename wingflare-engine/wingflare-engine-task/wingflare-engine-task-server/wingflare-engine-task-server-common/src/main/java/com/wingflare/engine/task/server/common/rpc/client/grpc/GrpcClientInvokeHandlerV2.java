...
import com.wingflare.engine.task.server.common.rpc.client.annotation.Param;
...
public class GrpcClientInvokeHandlerV2 implements InvocationHandler {
    ...
    public Result invoke(final Object proxy, final Method method, final Object[] args) throws Throwable {
        ...
        Assert.notNull(annotation, () -> new TaskServerException("@Mapping cannot be null"));
    ...
    private Result requestRemote(Method method, Object[] args, Mapping mapping) throws Throwable {
        ...
        try {
            ...
            if (RequestMethod.POST.name().equals(mapping.method().name())) {
                Assert.notNull(parasResult.body, () -> new TaskServerException("body cannot be null"));
            }
            ...
            Result result = retryer.call(() -> {
                ...
                if (async) {
                    ...
                    Assert.notNull(future, () -> new TaskServerException("completableFuture is null"));
                    try {
    ...
    private void failoverHandler(RegisterNodeInfo nodeInfo, InstanceManager instanceManager) {
        ...
        InstanceSelectCondition condition = Builder.of(InstanceSelectCondition::new)
                .with(InstanceSelectCondition::setAllocKey, allocKey)
                .with(InstanceSelectCondition::setGroupName, nodeInfo.getGroupName())
                .with(InstanceSelectCondition::setNamespaceId, nodeInfo.getNamespaceId())
                .with(InstanceSelectCondition::setRouteKey, routeKey)
                .with(InstanceSelectCondition::setTargetLabels, targetLabels)
                .build();
    ...
    private ParseParasResult doParseParams(Method method, Object[] args) {
        ...
        for (int i = 0; i < parameters.length; i++) {
            ...
            if (parameter.isAnnotationPresent(Body.class)) {
                ...
                throw new TaskServerException("parameter error");
            }
        }
