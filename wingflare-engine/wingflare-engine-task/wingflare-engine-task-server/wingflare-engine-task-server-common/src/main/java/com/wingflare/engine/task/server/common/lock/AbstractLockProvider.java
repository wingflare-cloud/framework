package com.wingflare.engine.task.server.common.lock;
...
public abstract class AbstractLockProvider implements LockProvider {
    ...
    public boolean lock(Duration lockAtLeast, Duration lockAtMost) {
        ...
        Assert.notNull(lockAtMost,
                () -> new TaskServerException("lockAtMost can not be null. lockName:[{}]", lockName));
        Assert.isFalse(lockAtMost.isNegative(),
                () -> new TaskServerException("lockAtMost  is negative. lockName:[{}]", lockName));
        Assert.notNull(lockAtLeast,
                () -> new TaskServerException("lockAtLeast can not be null. lockName:[{}]", lockName));
        Assert.isFalse(lockAtLeast.compareTo(lockAtMost) > 0,
                () -> new TaskServerException("lockAtLeast is longer than lockAtMost for lock. lockName:[{}]",
                        lockName));
    ...
    public void unlock() {
        try {
            LockConfig lockConfig = LockManager.getLockConfig();
            Assert.notNull(lockConfig, () -> new TaskServerException("lockConfig can not be null."));
            doUnlock(lockConfig);
        } finally {
            LockManager.clear();
        }
