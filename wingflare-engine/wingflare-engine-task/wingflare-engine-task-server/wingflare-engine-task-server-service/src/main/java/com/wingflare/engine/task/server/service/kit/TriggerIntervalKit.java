package com.wingflare.engine.task.server.service.kit;
 ...
 * @date 2025-07-05
 */
public final class TriggerIntervalKit {
    ...
    public static void checkTriggerInterval(String triggerInterval, Integer triggerType) {
        if (Objects.equals(triggerType, WaitStrategies.WaitStrategyEnum.POINT_IN_TIME.getType())) {
            ...
            if (first.isBefore(now)) {
                throw new TaskServerException("The submission time is less than the current time. triggerTime:{} now:{}", first, now);
            }

            // 判断间隔是否大于10秒
            Assert.isTrue(areAllIntervalsLessThan(localDateTimes, Duration.ofSeconds(10)),
                    () -> new TaskServerException("There are combinations with intervals less than 10(s)"));
        }
    }
