...
public class NamespaceServiceImpl implements NamespaceService {
    ...
    public Boolean saveNamespace(final NamespaceRequestVO namespaceRequestVO) {
        ...
        if (StrUtil.isNotBlank(namespaceRequestVO.getUniqueId())) {
            ...
            Assert.isTrue(matcher.matches(), () -> new TaskServerException("Only supports 1~64 characters, including numbers, letters, underscores, and hyphens"));
            ...
            Assert.isTrue(namespaceMapper.selectCount(
                    ...
                    () -> new TaskServerException("Namespace already exists {}", namespaceRequestVO.getUniqueId()));
        }
    ...
    public Boolean updateNamespace(final NamespaceRequestVO namespaceRequestVO) {
        ...
        Assert.notNull(id, () -> new TaskServerException("Parameter error"));
    ...
    public Boolean deleteByUniqueId(String uniqueId) {
        ...
        Assert.isFalse(DEFAULT_NAMESPACE.equals(uniqueId), ()-> new TaskServerException("Default space cannot be deleted"));
        ...
        Assert.isTrue(CollUtil.isEmpty(groupConfigMapper.selectList(new PageDTO<>(1, 1), new LambdaQueryWrapper<GroupConfig>()
                ...
                () -> new TaskServerException("There are undeleted group configuration tasks. Please delete the associated group configurations before retrying deletion"));

        Assert.isTrue(1 == namespaceMapper.delete(new LambdaQueryWrapper<Namespace>().eq(Namespace::getUniqueId, uniqueId)),
                () -> new TaskServerException("Failed to delete namespace"));
