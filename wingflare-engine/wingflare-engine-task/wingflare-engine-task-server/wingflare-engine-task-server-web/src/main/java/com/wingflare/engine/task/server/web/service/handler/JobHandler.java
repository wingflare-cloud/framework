...
import com.wingflare.engine.task.server.job.enums.JobRetrySceneEnum;
...
public class JobHandler {
    ...
    public Boolean retry(Long taskBatchId, Long workflowNodeId, Long workflowTaskBatchId) {
        ...
        Assert.notNull(jobTaskBatch, () -> new TaskServerException("job batch can not be null."));
        ...
        Assert.isTrue(jobTaskBatchMapper.updateById(jobTaskBatch) > 0,
                () -> new TaskServerException("update job batch to running failed."));
        ...
        for (JobTask jobTask : jobTasks) {
            ...
            Assert.isTrue(jobTaskMapper.updateById(jobTask) > 0,
                    () -> new TaskServerException("update job task to running failed."));
            // 模拟失败重试
    ...
    public void deleteJobTaskBatchByIds(Set<Long> ids, String namespaceId) {
        ...
        Assert.isTrue(ids.size() == jobTaskBatchMapper.deleteByIds(ids),
                () -> new TaskServerException("Failed to delete task batches"));
