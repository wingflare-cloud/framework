...
public class AuthenticationInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, Object object) throws Exception {
        ...
        if (loginRequired.required()) {
            // 执行认证
            if (token == null) {
                throw new TaskAuthenticationException("Login expired, please login again");
            }

            if (StrUtil.isBlank(namespaceId)) {
                throw new TaskAuthenticationException("Namespace {} does not exist", namespaceId);
            }
            ...
            if (Objects.isNull(systemUser)) {
                throw new TaskAuthenticationException("User does not exist");
            }

            Long count = namespaceMapper.selectCount(
                    new LambdaQueryWrapper<Namespace>().eq(Namespace::getUniqueId, namespaceId));
            Assert.isTrue(count > 0, () -> new TaskAuthenticationException("Namespace [{}] does not exist", namespaceId));
            ...
            if (userSessionVO.isUser()) {
                ...
                Assert.notEmpty(groupNames, () -> new TaskAuthenticationException("User group permissions are empty"));
            ...
            try {
                ...
                throw new TaskAuthenticationException("Login expired, please log in again");
            }
            ...
            if (role == RoleEnum.ADMIN) {
                if (role != RoleEnum.getEnumTypeMap().get(systemUser.getRole())) {
                    throw new TaskAuthenticationException("Lack of access permission");
                }
            }
