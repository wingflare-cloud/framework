...
import com.wingflare.engine.task.server.web.service.convert.PermissionsResponseVOConverter;
import com.wingflare.engine.task.server.web.service.convert.SystemUserResponseVOConverter;
...
import com.wingflare.task.datasource.template.persistence.mapper.SystemUserPermissionMapper;
...
public class SystemUserServiceImpl implements SystemUserService {
    ...
    private static void validateUserPassword(SystemUserRequestVO requestVO, SystemUser systemUser) {
        if (Objects.isNull(systemUser)) {
            throw new TaskServerException("Incorrect username or password");
        }
    ...
    public void addUser(SystemUserRequestVO requestVO) {
        ...
        if (count > 0) {
            throw new TaskServerException("User already exists");
        }
        ...
        Assert.isTrue(1 == systemUserMapper.insert(systemUser), () -> new TaskServerException("Failed to add user"));
        ...
        for (UserPermissionRequestVO permission : groupNameList) {
            ...
            Assert.isTrue(1 == systemUserPermissionMapper.insert(systemUserPermission),
                    () -> new TaskServerException("Failed to add user permissions"));
        }
    ...
    public void update(SystemUserRequestVO requestVO) {
        ...
        if (Objects.isNull(systemUser)) {
            throw new TaskServerException("User does not exist");
        }
        ...
        Assert.isTrue(1 == systemUserMapper.updateById(systemUser), () -> new TaskServerException("Failed to update user"));
        ...
        for (UserPermissionRequestVO permission : permissions) {
            ...
            systemUserPermission.setNamespaceId(permission.getNamespaceId());
            Assert.isTrue(1 == systemUserPermissionMapper.insert(systemUserPermission),
                    () -> new TaskServerException("Failed to update user permissions"));
        }
    }
    ...
    public void updateUserPassword(SystemUpdateUserPasswordRequestVO requestVO) {
        ...
        if (!SecureUtil.sha256(requestVO.getOldPassword()).equals(systemUser.getPassword())) {
            throw new TaskServerException("Incorrect original user password");
        }
        systemUser.setPassword(SecureUtil.sha256(requestVO.getNewPassword()));
        Assert.isTrue(1 == systemUserMapper.updateById(systemUser), () -> new TaskServerException("Failed to update user password"));
    }
