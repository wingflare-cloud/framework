...
public class RegisterJobExecutorsHttpRequestHandler extends GetHttpRequestHandler {
    ...
    public RegisterJobExecutorsHttpRequestHandler(AccessTemplate accessTemplate, DistributedLockHandler distributedLockHandler) {
    ...
    public TaskRpcResult doHandler(String content, UrlQuery query, HttpHeaders headers) {
        ...
        if (arg == null) {
            return new TaskRpcResult(StatusEnum.NO.getStatus(), "Beat register job executors arg is null", Boolean.FALSE, retryRequest.getReqId());
        }
        ...
        return new TaskRpcResult(StatusEnum.YES.getStatus(), "Batch Register Job Executors Processed Successfully", Boolean.TRUE, retryRequest.getReqId());
    }
