<template>
  <div>
    ...
    <el-dialog
        ...
        top="5vh"
    >
      <el-form :rules="rules" :model="genSetting" label-width="220px">
        <el-form-item prop="author" label="代码作者">
          <el-input v-model="genSetting.author" style="width: 260px"></el-input>
        </el-form-item>
        <el-form-item prop="basePackage" label="基础包名">
          <el-input
              v-model="genSetting.basePackage"
              placeholder="项目的基础包名"
              style="width: 400px"
          ></el-input>
        </el-form-item>
        <el-form-item prop="moduleName" label="功能模块名">
          <el-input
        ...
        <el-form-item prop="choosedOutputFiles" label="本次需生成的文件">
        ...
        <el-form-item prop="isControllerChecked" label="需生成的业务方法" v-if="isControllerChecked">
          <el-alert
              title="注意：如果更换了默认模板，以下选项可能不会生效，需自行在模板中实现"
              type="warning"
          ></el-alert>
          <el-checkbox-group v-model="genSetting.choosedControllerMethods">
            <el-checkbox label="list" key="list">列表查询</el-checkbox>
            <el-checkbox label="get" key="get">按ID查询</el-checkbox>
            <el-checkbox label="getOnlyOne" key="getOnlyOne">按条件查单个</el-checkbox>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        basePackage: [
          {required: true, message: "请输入基础包名", trigger: "change"},
        ],
        moduleName: [
          {required: true, message: "请输入模块名", trigger: "change"},
        ],
        rootPath: [
          {required: true, message: "请输入文件生成目录", trigger: "change"},
        ],
      }
    };
  },
    ...
    isControllerChecked: function () {
      if (this.genSetting.choosedOutputFiles) {
        if (this.genSetting.choosedOutputFiles.indexOf("Controller") !== -1
            || this.genSetting.choosedOutputFiles.indexOf("BizImpl") !== -1
            || this.genSetting.choosedOutputFiles.indexOf("Biz") !== -1) {
          return true;
        } else {
          return false;
        }
  ...
  methods: {
    ...
    openGenSetting() {
      ...
      try {
        ...
        if (!this.genSetting.basePackage) {
          axios.get("/api/output-file-info/project-base-pkg").then((res) => {
            this.genSetting.basePackage = res;
          });
        }
      } catch (e) {
        console.error(e);
